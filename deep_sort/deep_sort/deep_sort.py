...
class DeepSort(object):
    ...
    def update(self, bbox_xywh, confidences, ori_img, cls_id):
        ...
        detections = [Detection(bbox_tlwh[i], conf, features[i], cls_id[i]) for i,conf in enumerate(confidences) if conf>self.min_confidence]

        # run on non-maximum supression
        boxes = np.array([d.tlwh for d in detections])
        scores = np.array([d.confidence for d in detections])
        class_id = np.array([d.cls_id for d in detections])
        indices = non_max_suppression(boxes, self.nms_max_overlap, scores, class_id)
        ...
        # body_num = len([d.cls_id for d in detections if d.cls_id==0])
        ...
        for track in self.tracker.tracks:
            ...
            # box = track.to_tlwh()
            # track_id = track.track_id
            # outputs.append(np.array([box,track_id], dtype=np.int))
